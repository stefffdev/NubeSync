...
public partial class NubeClient
{
    ...
    {
        return (await _dataStore.GetOperationsAsync().ConfigureAwait(false)).Any();
    }
    ...
    public async Task<int> PullTableAsync<T>(CancellationToken cancelToken = default) where T : NubeTable
    {
        ...
        try
        {
            ...
            do
            {
                ...
                var content = await result.Content.ReadAsStringAsync(cancelToken).ConfigureAwait(false);
                items = JsonSerializer.Deserialize<List<T>>(content, _jsonOptions) ?? [];
                ...
                pageNumber++;
            } while (items.Count != 0 && items.Count == PULL_PAGE_SIZE);
    ...
    public async Task<bool> PushChangesAsync(CancellationToken cancelToken = default)
    {
        ...
        try
        {
            ...
            while (operations.Any())
            {
                ...
                var content = new StringContent(JsonSerializer.Serialize(operations, _jsonOptions),
